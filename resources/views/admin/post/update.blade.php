...
@section('content')
    <div class="col mt-5">
        <div class="row">
            <div class="col-4 ">
                <form action="{{ route('admin#updatePost', $postDetails['post_id']) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="div">
                        Title: <input type="text" name="title" value="{{ old('title', $postDetails['title']) }}"
                        ...
                        <textarea name="description"
                            ...
                            is-invalid
                        @enderror"
                            cols="30" rows="5" id="" placeholder="Enter post description">{{ old('description', $postDetails['description']) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror

                        Image:<br>
                        <img class="rounded shadow" style="width: 50%"
                            @if ($postDetails['image'] == null) src="{{ asset('defaultImage/defaultImage.png') }}"
                            @else
